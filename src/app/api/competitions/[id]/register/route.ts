...
export async function POST(
  ...
  try {
    ...
    if (!team) {
      return NextResponse.json(
        { error: 'Invalid Team_ID / No team found' },
        { status: 404 }
      );
    }
    ...
    if (dt_now > comp.registration_deadline) {
      return NextResponse.json(
        { error: 'Registrations are closed now!' },
        { status: 400 }
      );
    }
    ...
    if (team_size < comp.min_team_size) {
      return NextResponse.json(
        { error: `Minimum of ${comp.min_team_size} required!` },
        { status: 400 }
      );
    }
    if (team_size > comp.max_team_size) {
      return NextResponse.json(
        { error: `Maximum of ${comp.max_team_size} required!` },
        { status: 400 }
      );
    }
    ...
    if (users_registered.length > 0) {
      return NextResponse.json(
        {
          error: 'Some members already registered!',
    ...
    const members = team.team_members.map((member) => member.id);
    ...
    return NextResponse.json({
      status: 200,
      message: `Team ${team.team_name} successfully registered to ${comp.competitionName}`,
    });
  } catch (e) {
    return NextResponse.json(
