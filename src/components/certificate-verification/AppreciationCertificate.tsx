...
import { Award, Calendar, User, Hash, BookOpen, Trophy } from 'lucide-react';

interface AppreciationCertificateProps {
  certificate: Certificate;
}

const AppreciationCertificate: React.FC<AppreciationCertificateProps> = ({ certificate }) => {

    let formattedDate = null;

    if (certificate.issuedDate) {
        formattedDate = new Date(certificate.issuedDate).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
        });
    ...
    >
        ...
        <div className="px-6 py-5">
            ...
            <h3 className="text-xl font-bold text-white">{certificate.issuedFor}</h3>
            </div>
            ...
            <div>
                ...
                <div>
                    ...
                    <p className="font-semibold text-white">{certificate.issuedTo}</p>
                </div>
                </div>
                ...
                <div>
                    ...
                    {formattedDate && <p className="font-medium text-gray-300">{formattedDate}</p>}
                </div>
                </div>
                ...
                <div className="flex items-center mb-3">
                    ...
                    <p className="font-medium text-gray-300">{certificate.branch}</p>
                    </div>
                </div>
                )}
            </div>
            ...
            <div>
                ...
                <div className="flex items-center mb-3">
                    ...
                    <p className="font-medium text-gray-300">{certificate.roll_number}</p>
                    </div>
                </div>
                )}
                ...
                <div className="flex items-center mb-3">
                    ...
                    <p className="font-medium text-gray-300">{certificate.SerialNumber}</p>
                    </div>
                </div>
                )}
                ...
                <div className="flex items-center mb-3">
                    ...
                    <p className="font-medium text-gray-300">
                        {certificate.ranking === 1 ? '1st' : 
                        certificate.ranking === 2 ? '2nd' : 
                        certificate.ranking === 3 ? '3rd' : 
                        `${certificate.ranking}th`} Place
                    </p>
                    </div>
                </div>
                )}
