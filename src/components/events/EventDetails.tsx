    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-black/30 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-800">
        ...
        <div className="p-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400">{new Date(event.conductedOn).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400">{new Date(event.conductedOn).toLocaleTimeString()}</p>
                </div>
              </div>
          ...
          {/* Prize Details */}
          {(event.first_prize || event.second_prize || event.third_prize || event.prize) && (
            <div className="mb-8">
              ...
              <div className="bg-black/30 rounded-lg p-6 border border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {event.first_prize && (
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-yellow-500/20 flex items-center justify-center text-yellow-500">1</div>
                      <span>{event.first_prize.name}</span>
                    </div>
                  )}
                  {event.second_prize && (
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gray-400/20 flex items-center justify-center text-gray-400">2</div>
                      <span>{event.second_prize.name}</span>
                    </div>
                  )}
                  {event.third_prize && (
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-orange-500/20 flex items-center justify-center text-orange-500">3</div>
