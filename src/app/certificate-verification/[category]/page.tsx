...
export default async function Certificates_Page({
  params,
}: {
  ...
  const appreciationCertificates = certificateData.filter(
    (cert) => cert.cop === false
  );
  const participationCertificates = certificateData.filter(
    (cert) => cert.cop === true
  );
      ...
      <main className="relative z-10 container mx-auto px-4 py-20 min-h-[80vh]">
        <div className="mt-6 mb-12">
          ...
          {participationCertificates.length > 0 && (
            <div>
              <ParticipationCertificateList
                certificates={participationCertificates}
              />
            </div>
          )}
...
export async function getCertificateData(categoryName: string) {
  try {
    ...
    const response = await fetch(
      `${process.env.API_URL}/api/certificate-verification/${categoryName}`
    );
